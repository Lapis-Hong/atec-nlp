...
tf.flags.DEFINE_string("word2vec_file", "data/w2v.txt", "Word2vec pre-trained embeddings. (default: None)")
...
def train():
    with tf.Graph().as_default():
        ...
        train_data, eval_data = dataset.train_test_split(zip(*data), test_size=FLAGS.val_percentage, random_seed=FLAGS.random_seed)
        train_batches = dataset.batch_iter(list(zip(*train_data)), FLAGS.batch_size, FLAGS.num_epochs, shuffle=True)
        ...
        for batch in train_batches:
            ...
            if step % FLAGS.evaluate_every_steps == 0:
                # eval
                time_str = datetime.datetime.now().isoformat()
                x1_batch, x2_batch, y_batch, seqlen1, seqlen2 = eval_data
                ...
                x1, out1, out2, sim_euc, sim_cos, sim_ma = sess.run([model.embedded_chars1, model.out1, model.out2, model.sim_euc, model.sim_cos, model.sim_ma], feed_dict)
                # print(x1)
                ...
                print(zip(out1, out2))
                for w in zip(y_batch[:30], sim_euc, sim_cos, sim_ma):
                    print(w)
